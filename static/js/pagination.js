	...
	return this.each(function() {
		...
		function drawLinks() {
			...
			if (opts.display_msg) {
				if(!maxentries){
					panel
						.append('<div class="pxofy">No Data</div>');
				}else{
				panel
						.append('<div class="pxofy">show from&nbsp;'
								+ ((current_page * opts.items_per_page) + 1)
								+ '&nbsp;items to&nbsp;'
								+ (((current_page + 1) * opts.items_per_page) > maxentries
										? maxentries
										: ((current_page + 1) * opts.items_per_page))
								+ '&nbsp;items，all&nbsp;' + maxentries + '&nbsp;items</div>');
				}
			}
			//设置跳到第几页
			if(opts.setPageNo){
				  panel.append("<div class='goto'><span class='text'>jump to</span><input type='text'/><span class='page'>page</span><button type='button' class='ue-button long2'>submit</button></div>");
			}
		}
		...
		if(maxentries==0){
			panel.append('<span class="current prev">'+opts.prev_text+'</span><span class="current next">'+opts.next_text+'</span><div class="pxofy">No Data</div>');
		}else{
			drawLinks();
		}
